...
public class RookieServiceTest extends AbstractTest {
	...
	/**
	 * Este test hace referencia al registro de un nuevo Rookie.
	 ...
	 */
	@Test
	public void RookieRegisterDriver() throws ParseException {
		...
		for (int i = 0; i < testingData.length; i++)
			this.RookieRegisterTemplate((String) testingData[i][0], (boolean) testingData[i][1], (Class<?>) testingData[i][2]);
	}

	// Ancillary methods ------------------------------------------------------

	protected void RookieRegisterTemplate(final String username, final boolean validData, final Class<?> expected) throws ParseException {
		Class<?> caught;

		caught = null;
		Rookie admin = this.rookieService.create();
		try {
			...
			admin = RookieServiceTest.fillData(admin);
			this.rookieService.save(admin);
	...
	private static Rookie fillData(final Rookie admin) {
		final Rookie res = admin;
