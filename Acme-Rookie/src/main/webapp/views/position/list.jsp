...
<display:table pagesize="5" name="positions" id="position" requestURI="${requestURI}">
   		 ...
   		 <jstl:if test="${owner}">
   			 ...
   			 <display:column titleKey="position.list.changeDraft">
   				 ...
   				 <jstl:if test="${position.draft && !positionsChangeDraft.contains(position)}">
   					 <spring:message code="position.company.contraint.2problems"/>
   				 </jstl:if>
   			 </display:column>
   			 ...
   			 <display:column titleKey="position.list.cancel">
   				 <jstl:if test="${!position.draft && position.cancelled}">
   					 <acme:button url="position/company/changeCancellation.do?idPosition=${position.id}" type="button" code="position.list.uncancel"/>
   				 </jstl:if>
   				 <jstl:if test="${!position.draft && !position.cancelled}">
