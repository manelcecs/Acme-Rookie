...
public interface PositionRepository extends JpaRepository<Position, Integer> {
	...
	@Query("select p from Position p where p.draft = false and p.cancelled = false and p.deadline > CURRENT_DATE and (select count(pt) from Application a join a.problem.position pt where a.rookie.id = ?1 and pt.id = p.id) * 1 = 0")
	Collection<Position> getPositionsCanBeApplied(int idRookie);
