...
public interface ProviderRepository extends JpaRepository<Provider, Integer> {
	...
	@Query("select distinct p from Sponsorship s join s.provider p where (select count(s) from Sponsorship s join s.provider pr where pr.id = p.id)>=(select avg(1*(select count(s) from Sponsorship s where s.provider.id = pro.id)) from Provider pro)")
