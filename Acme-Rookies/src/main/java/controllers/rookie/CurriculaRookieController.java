...
public class CurriculaRookieController extends AbstractController {
	...
	protected ModelAndView createModelAndViewDisplay(final Integer curriculaId) {
		...
		try {
			...
			Assert.isTrue(this.rookieService.findByPrincipal(LoginService.getPrincipal()).getId() == curricula.getRookie().getId());
	...
	protected ModelAndView createModelAndViewList() {
		...
		try {
			...
			final List<Curricula> curriculas = (List<Curricula>) this.curriculaService.findAllNoCopy(rookie);
