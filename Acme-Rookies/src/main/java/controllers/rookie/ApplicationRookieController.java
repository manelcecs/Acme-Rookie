...
public class ApplicationRookieController extends AbstractController {
	...
	public ModelAndView list() {
		...
		final Collection<Application> applicationsAnswered = this.applicationService.getApplicationsAnswered(rookie.getId());
	...
	public ModelAndView listByStatus(@RequestParam(required = true) final String status) {
		...
		final Collection<Application> applications = this.applicationService.getApplicationOfRookieByStatus(rookie.getId(), status);
		final Collection<Application> applicationsAnswered = this.applicationService.getApplicationsAnswered(rookie.getId());
	...
	public ModelAndView display(@RequestParam(required = true) final int idApplication) {
		...
		if (application.getRookie().getId() != rookie.getId())
	...
	protected ModelAndView createModelAndView(final ApplicationForm applicationForm, final String message) {
		...
		final Rookie rookie = this.rookieService.findByPrincipal(LoginService.getPrincipal());
		final Collection<Position> positions = this.positionService.getPositionsCanBeApplied(rookie.getId());
		final Collection<Curricula> curriculas = this.curriculaService.findAllNoCopy(rookie);
